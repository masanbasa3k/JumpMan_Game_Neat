...
def main(genomes, config):
    ...
    while run and len(players) > 0:
        ...
        for x, player in enumerate(players):
            ...
            output = nets[x].activate((player.y, abs(player.y - pipes[pipe_ind].height), abs(player.y - pipes[pipe_ind].bottom)))
        ...
        for x, player in enumerate(players):
            if player.y + player.img.get_height() >= 730 or player.y < 0:
                ge[x].fitness -= 1
                players.pop(x)
                nets.pop(x)
                ge.pop(x)
